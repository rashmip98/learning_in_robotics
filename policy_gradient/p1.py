...
def train(num_traj = 10, T=1000):
    ...
    for i in range(T):
        r = th.zeros(num_traj)
        log_p = th.zeros((num_traj,200))
        for j in range(num_traj):
            t = rollout(policy)
            r[j] = t['R']
            log_p[j, :] = policy(t['x'].view(-1,2), t['u'].view(-1,1))[1].squeeze()
    
        baseline = r.mean()
        reward.append(baseline)

        #print(r.shape, log_p.shape)
        f = -((r - baseline)* log_p.T).mean()
        policy.zero_grad()
        f.backward()
        optim.step()
        #print('Iteration: ', i)
